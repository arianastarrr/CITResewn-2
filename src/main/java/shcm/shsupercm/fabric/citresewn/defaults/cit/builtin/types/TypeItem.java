package shcm.shsupercm.fabric.citresewn.defaults.cit.builtin.types;
...
import shcm.shsupercm.fabric.citresewn.defaults.cit.builtin.conditions.ItemCondition;
...
import shcm.shsupercm.fabric.citresewn.defaults.mixin.types.item.ItemRendererMixin;
...
public class TypeItem extends CITType {
    ...
    public void load(List<CITCondition> conditions, PropertyGroup properties, ResourceManager resourceManager) throws CITParsingException {
        for (CITCondition condition : conditions)
            if (condition instanceof ItemCondition conditionItems)
        ...
        if (this.items.size() == 0)
            try {
                ...
                conditions.add(new ItemCondition(item));
                this.items.add(item);
        ...
        public void load(List<CIT<TypeItem>> parsedCITs) {
            ...
            for (CIT<TypeItem> cit : parsedCITs)
                for (CITCondition condition : cit.conditions)
                    if (condition instanceof ItemCondition items)
